...
int InnoTalkNs_ProcessCore(NSinst_t* inst, short* speechFrame, short* outFrame){
  ...
  for (i=0;i<inst->anaLen;i++)
  {
	...
	CEVA_DSP_LIB_FLOAT_FFT_CX_OOB((int32_t)inst->stages, CFFT_Inns, CFFT_Outns, CEVA_DSP_LIB_FLOAT_cos_sin, (int16*)bitrev_32_1024, tmp, 1);
  // 计算当前帧的功率谱和相位谱
...
#if FRAME128
	CEVA_DSP_LIB_FLOAT_FFT_REAL_INV_OOB((int32_t)inst->stages, CFFT_Outns, CFFT_InvOutns, CEVA_DSP_LIB_FLOAT_cos_sin, twi_table_float_rfft_256, bitrev_32_1024, tmp, 1);
#else
  CEVA_DSP_LIB_FLOAT_FFT_REAL_INV_OOB((int32_t)inst->stages, CFFT_Outns, CFFT_InvOutns, CEVA_DSP_LIB_FLOAT_cos_sin, twi_table_float_rfft_128, bitrev_32_1024, tmp, 1);
#endif
