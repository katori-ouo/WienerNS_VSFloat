...
void CEVA_DSP_LIB_FLOAT_FFT_CX_STAGEN_OOB(
	int32 fft_size,
	float_32 *in_buf,
	float_32* in_buf_2,
	float_32 *out_buf,
	...
	float_32 const *twi_table)
{
    ...
    for (i = 0; i<fft_size / 2; i++)
    {
        ...
        out_buf[4 * i + 0] = ar + itemp;
        out_buf[4 * i + 1] = ai - rtemp;
        out_buf[4 * i + 2] = ar - itemp;
        out_buf[4 * i + 3] = ai + rtemp;
...
.. c:function::  void CEVA_FFT_LIB_CX32_FFT_OOB(int32 log2_buf_len, int32 *in_buf, int32 *out_buf, int32 const *twi_table, int16 *bitrev_tbl, int32 *temp_buf, int8 *ScaleShift, int32 br)
    ...
    )
{
	...
	{
		CEVA_DSP_LIB_FLOAT_FFT_CX_STAGEN_OOB(fft_size, (float_32*)buf1, (float_32*)(buf1 + fft_size), (float_32*)buf2, modu_fac, ptrTwiCycl);
		modu_fac <<= 1;
		buf_t = buf1;
		buf1 = buf2;
		buf2 = buf_t;
